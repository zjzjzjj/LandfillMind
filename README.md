...
> 主题：为人民建好房，为工友谋幸福 ｜ 参赛者：郑杰（浙江大学环境岩土工程）
...
- **黄金值回归测试**：`npx tsx tests/ogs-models.test.ts`（默认参数 CH₄ 年累计 ≈16457 万m³ ±2%、日产峰值 ≈899 万m³/d、沉降末端 ≈−0.0199 m）
- **AI 场景生成测试**：`npm run test:scene`（47 条断言：斜坡比/中文数字/亿级规模/key=value/意图判定/疑问句拦截/OGS 成败/clamp 兜底/NaN 消毒）
